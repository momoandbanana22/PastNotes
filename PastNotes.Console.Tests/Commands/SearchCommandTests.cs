...
public class SearchCommandTests
{
    ...
    public async Task ExecuteAsync_WhenStartDateAfterEndDate_ThrowsArgumentException()
    {
        ...
        var testNotes = new List<Note>
        {
            new Note { Id = "1", Text = "Test note", CreatedAt = new DateTime(2024, 1, 15, 0, 0, 0, DateTimeKind.Utc) }
        };
        await repository.SaveToFileAsync(testNotes, testFilePath);
        ...
        {
            // Act & Assert
            await Assert.ThrowsAsync<ArgumentException>(() => command.ExecuteAsync("Test"));
        }
        finally
        {
            if (File.Exists(testFilePath)) File.Delete(testFilePath);
