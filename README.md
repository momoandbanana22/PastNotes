...
#### 方法1: 単一の .exe をビルドして使う（推奨・実行時に .NET SDK 不要）

- [.NET 10 SDK](https://dotnet.microsoft.com/download/dotnet/10.0) がインストールされていること（ビルド時のみ必要）

以下のコマンドで自己完結型の単一 .exe をビルドします。

    dotnet publish PastNotes.Console/PastNotes.Console.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -o ./publish

`.\publish\PastNotes.exe` が生成されるので、以降はこれを実行するだけで使えます（.NET SDK のインストール不要）。

    .\publish\PastNotes.exe fetch --days 30

以降の使用方法の説明では `dotnet run --project PastNotes.Console/PastNotes.Console.csproj --` の部分を `.\publish\PastNotes.exe` に読み替えてください。

#### 方法2: `dotnet run` でソースから直接実行する
...
`MISSKEY_INSTANCE_URL` のデフォルトは `https://misskey.io` です。misskey.io を使用している場合は省略できます。

```powershell
$env:MISSKEY_INSTANCE_URL = "https://misskey.io"  # misskey.io の場合は省略可
$env:MISSKEY_API_TOKEN = "your-api-token"
```

**方法3: `.env` ファイル（初回設定に便利）**

プロジェクトルートに `.env` ファイルを作成します（`.gitignore` で除外済みのためコミットされません）。
