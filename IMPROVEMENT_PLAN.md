...
### [ ] DESIGN-3. `SearchCommand`・`ViewCommand`・`ViewHtmlCommand` の例外が `Program.cs` 汎用ハンドラ経由で stdout に出力される
...
**修正案**: `SearchCommand`・`ViewCommand`・`ViewHtmlCommand` にも `catch (Exception ex)` を追加して `Console.Error.WriteLine` で出力するか、`Program.cs` の汎用ハンドラを `Console.Error.WriteLine` に変更する。
