...
**問題ではないことの確認**: `publish/PastNotes.exe`（REL-1 でビルドした自己完結型 .exe）を Bash・PowerShell の両方で直接実行し、`view --show-id`（「添付ファイル:」を含む日本語出力）が文字化けせず正しく表示されることをコマンド実行で確認した。したがって BUG-40 の修正（`Program.cs` の `Main` 冒頭で `Console.OutputEncoding = UTF8` を設定）は実際のエンドユーザー実行経路では機能している。

**根本原因**: `dotnet test` 内の文字化けは、統合テストが `FetchCommand`・`MisskeyApiClient` などのクラスを直接呼び出しており `Program.Main` を経由しないために起きる。テストプロセス（`testhost.exe`）自身のコンソールエンコーディングは Windows の既定値（CP932）のままなので、日本語文字列（`取得中... {n} 件` 等）を `Console.WriteLine` するとテストログ上でのみ文字化けする。これは CLAUDE.md の「テスト環境と本番環境の差異」表・「起動・初期化」行に既に記載されている既知の差異であり、TST-30（`Main_WhenStarted_SetsConsoleOutputEncodingToUtf8`）はこの差異を踏まえた上で `Program.Main` 経路のみを検証する設計になっている。
...
### [x] DOC-7. `fetch` の `--days` と `--start`/`--end` 同時指定時のエラー仕様が README・DEVELOPMENT.md に記載されていない（BUG-44 の文書化漏れ）

**対象ファイル**: `README.md`、`DEVELOPMENT.md`

**問題**: BUG-44 で `fetch --days` と `fetch --start`/`--end` を同時指定すると `Error: --days cannot be used together with --start/--end` を出力して exit 1 する仕様が `FetchCommandHandler.cs`（59〜63行目）に実装されたが、README.md・DEVELOPMENT.md のどちらにもこの仕様の記載がなかった。DOC-4（`--max-retries`）では同様の「後から追加された挙動」がドキュメント化されたが、BUG-44 はコード変更のみで完結し対応する DOC 項目が作成されていなかった。

**対処**: README.md の「取得系コマンド」セクションと「運用上の注意事項」、DEVELOPMENT.md の「ビルドと実行」の例にそれぞれ `--days` と `--start`/`--end` が排他であることを追記した。
...
### [x] DESIGN-7. `Program.cs` に引数パース・DI・例外処理が全て集約されており肥大化している
...
**対処**: 修正案の2案のうち「コマンドごとにパーサークラスを切り出す」を採用した。`System.CommandLine` 等のライブラリ導入は、既存の66件超の `ConsoleAppTests` がエラーメッセージ・使用方法表示・終了コードを厳密に検証しており、ライブラリ側の解析仕様・出力フォーマットに合わせて多数のテスト・文言を書き換える必要が生じるため、リスクに見合わないと判断し見送った。

`PastNotes.Console/Cli/` を新設し、`FetchCommandHandler`・`SearchCommandHandler`・`ViewCommandHandler`・`ViewHtmlCommandHandler` の4クラスに、各コマンドの引数パース・DI・実行・例外処理を**一字一句変更せずに**移動した。`Program.cs` の `Main()` はコマンド名から対応するハンドラーを呼び出すだけになり、256行 → 58行に削減された。

これは動作を変えない構造変更のみのため、CLAUDE.md のリファクタリングルール（新規テスト不要、既存テストの GREEN 維持で安全性を確認）に従い、新規テストは追加していない。移動前後で `PastNotes.Console.Tests` 66件・`PastNotes.Tests` 68件が完全に同一件数で全件パスすることを確認し、動作が一切変わっていないことを確認した。`dotnet build` 警告0件。

---

### [x] DESIGN-8. 引数バリデーションエラーが stdout、例外エラーが stderr で不統一（DESIGN-3 の対処で可視化）

**対象ファイル**: `PastNotes.Console/Cli/FetchCommandHandler.cs`・`SearchCommandHandler.cs`・`ViewCommandHandler.cs`（DESIGN-7 で `Program.cs` から移動済み）

**問題**: DESIGN-3 で `fetch`・`search`・`view`・`view-html` の例外（`catch (Exception ex)`）を stderr に統一したが、`--token requires a value`・`Invalid start date format` 等、引数パース段階でのバリデーションエラー（15箇所、try/catch の外で直接 `Console.WriteLine` している）は stdout のまま残っている。同じ「ユーザーへのエラー通知」でも、検出タイミング（引数パース時 vs 実行時例外）によって出力先が異なる状態になった。
...
**対処**: TDD で対応。失敗テスト `SearchCommand_WhenStartFlagHasNoValue_PrintsErrorToStderr`（`search keyword --start` で値なしのとき、エラーが stderr に出て stdout には出ないことを検証）を追加し RED を確認した後、3ファイルの `Console.WriteLine("Error...")` 計15箇所を `Console.Error.WriteLine(...)` に統一して GREEN を確認した（`Usage:` から始まる使用方法表示はエラー通知ではないため対象外、stdout のまま維持）。

この変更により、同じシナリオを stdout 前提で検証していた既存13テスト（`FetchCommand_WhenTokenFlagHasNoValue_ReturnsOneAndPrintsError` 等）が意図通り RED になることを確認した上で、全て `Console.SetError` を使う形に更新した。更新後、追加した一時テストは既存テストと重複するため削除した。

横展開確認: `Console.WriteLine("Error` パターンを Grep 検索し、該当15箇所以外に残存しないことを確認済み。`PastNotes.Console.Tests` 66件・`PastNotes.Tests` 68件全ユニットテストパス（動作は意図通り変更、既存テストは新しい仕様に合わせて更新）、`dotnet build` 警告0件。

---

## REL: リリース整備

### [x] REL-1. ユーザー向けに .exe で実行できるようにする

**問題**: README の使用方法が `dotnet run --project ...` になっており、.NET SDK のインストールと長いコマンドが必要。エンドユーザーには不親切。

**対応**: `dotnet publish` でシングルファイルの自己完結型 .exe をビルドできることを確認し、README・DEVELOPMENT.md に手順を追記した。
...
**完了の判断基準と証拠**:
- 上記コマンドが実際に成功し `.\publish\PastNotes.exe` が生成されることをコマンド実行で確認した。
- 生成された .exe を `dotnet` を経由せず直接実行し（`view` コマンド）、`No notes found. Run 'fetch' command first.` が出力・終了コード1で返ることを確認した（単一実行ファイルとして正常に動作）。
- `publish/` は VisualStudio 標準テンプレートの `.gitignore` に既に含まれていることを確認済み（追加変更不要）。

**対応したファイル**:
- `README.md` — 「実行方法」セクションを新設し、`dotnet publish` でビルドした .exe を使う方法を追記（既存の `dotnet run` 手順も残し「方法2」として維持）
- `DEVELOPMENT.md` — 「配布用ビルド（publish）」セクションを追記

**exe ファイル名について**:
デフォルトでは `AssemblyName` がプロジェクト名（`PastNotes.Console`）由来になるため `PastNotes.Console.exe` が生成される。エンドユーザー向けの実行ファイル名としては `PastNotes.exe` の方が分かりやすいため、`PastNotes.Console.csproj` に `<AssemblyName>PastNotes</AssemblyName>` を追加した。

ただし単純に追加すると、ライブラリプロジェクト `PastNotes.csproj`（フォルダ名由来のデフォルト `AssemblyName` が同じく `PastNotes`）と衝突し、`dotnet restore` が `Ambiguous project name 'PastNotes'` で失敗することをコマンド実行で確認した。そのため `PastNotes.csproj` 側に `<AssemblyName>PastNotes.Core</AssemblyName>` を明示的に設定し、両者のアセンブリ名を分離した。

- コード内の `namespace PastNotes;` 等の名前空間宣言は明示的なため、`AssemblyName`（デフォルトの `RootNamespace` 由来）の変更による影響を受けないことを `grep` で確認済み。
- `dotnet build`（0エラー）・`dotnet test --filter "Category=Unit"`（76件成功）・`dotnet publish` → 生成された `PastNotes.exe` を直接実行して動作することをそれぞれコマンドで確認した。
